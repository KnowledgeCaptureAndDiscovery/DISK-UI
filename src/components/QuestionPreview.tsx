...
export const QuestionPreview = ({
  ...
  const [nameToValue, setNameToValue] = React.useState<{
    [id: string]: string;
  }>({});
  ...
  React.useEffect(() => {
    if (selectedQuestion && bindings.length > 0) {
      ...
      let noOptionalsPattern: string = selectedQuestion.pattern
    ...
    <Box>
      <Card
        ...
        sx={{
          mt: "8px",
          p: "0px 10px 10px;",
          visibility: questionParts.length > 0 ? "visible" : "collapse",
          position: "relative",
          overflow: "visible",
        }}
      >
        ...
        >
              ...
              ? questionParts.map((part: string, i: number, parts: string[]) =>
                  part.charAt(0) !== "?" ? (
                    ...
                    <>
                      ...
                      >
                        {nameToValue[part] ? nameToValue[part] : "any"}
                      </TextPart>
                      {i === (parts.length - 1) && (
          ...
          <Tooltip
            arrow
            title={(formalView ? "Hide" : "Show") + " formal expression"}
          >
        ...
        >
          ...
          >
            <Table sx={{ width: "unset" }}>
              <TableBody>
                {triplePattern.map((triple: string[], index: number) => (
                  <TableRow key={`row_${index}`}>
                    {triple.map((res: string) => (
                      <TableCell
                        key={`cell${index}${res}`}
                        sx={{ padding: "2px 10px" }}
                      >
