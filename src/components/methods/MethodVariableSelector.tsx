...
type DataInputSelectorIds = 'WORKFLOW_OUTPUT';
type ParamInputSelectorIds = 'WORKFLOW_DATE';
...
type DataInputFieldMap = InputFieldMap & {
    [key in DataInputSelectorIds]:  SelectorField;
} 
type ParamInputFieldMap = InputFieldMap & {
    [key in ParamInputSelectorIds]: SelectorField;
}
...
const DATA_INPUT_SELECTORS : DataInputFieldMap = {
    ...INPUT_SELECTORS,
    WORKFLOW_OUTPUT: {
        label: "Send files generated by this LOI",
        tooltip: "Send one or multiple files generated by this LOI in this and previous executions of this LOI."
    },
}

const PARAM_INPUT_SELECTORS : ParamInputFieldMap = {
    ...INPUT_SELECTORS,
    WORKFLOW_DATE: {
        label: "Send date of this and previous runs",
        tooltip: "Send one or multiple dates of execution for this and previous executions of this LOI."
    }
}
...
export const MethodVariableSelector = ({variable, options, value, onChange, meta=false, params=false, storedOutputs=[]}:MethodVariableProps) => {
    const [selected, setSelected] = useState<string>("");
    const [selectedType, setSelectedType] = useState<string>("");
    const [selectorKind, setSelectorKind] = useState<InputSelectorIds | DataInputSelectorIds | ParamInputSelectorIds>("DEFAULT_VALUE");
    ...
    const onSelectorChange = (newSelector:InputSelectorIds | DataInputSelectorIds | ParamInputSelectorIds) => {
    ...
    const renderBindingSelector = () => {
        ...
        switch (selectorKind) {
            ...
            case 'WORKFLOW_DATE':
                return <Box>Execution dates of all runs of this workflow will be send</Box>
            case 'DEFAULT_VALUE':
            default:
                return <Box>0</Box>
        }
    ...
    const SELECTED_INPUT_SELECTORS = meta ? (params? PARAM_INPUT_SELECTORS : DATA_INPUT_SELECTORS) : INPUT_SELECTORS;
            ...
            <Grid item xs={4} md={4} sm={4}>
                <Box style={{ display: "flex" }}>
                    <Select size="small" sx={{ display: 'inline-block', width: "100%"}} variant="standard" label="Type of binding"
                        ...
                        {Object.keys(SELECTED_INPUT_SELECTORS).map((id: string, i: number) => <MenuItem key={id} value={id}>{SELECTED_INPUT_SELECTORS[id as InputSelectorIds].label}</MenuItem>)}
                    </Select>
                    <Box style={{ zIndex: 10 }}>
                        <Tooltip arrow title={SELECTED_INPUT_SELECTORS[selectorKind as InputSelectorIds].tooltip}>
                            <HelpOutlineIcon/>
                        </Tooltip>
                    </Box>
                </Box>
