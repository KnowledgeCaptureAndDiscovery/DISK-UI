...
export const QuestionLinker = ({
  selected: selectedId,
  disabled,
  onQuestionChange: notifyChange,
  error: exError = false,
}: QuestionLinkerProps) => {
  const dispatch = useAppDispatch();
  const error = useAppSelector((state: RootState) => state.question.errorAll);
  const loading = useAppSelector(
    (state: RootState) => state.question.loadingAll
  );
  ...
  const [selectedQuestionLabel, setSelectedQuestionLabel] =
    ...
    if (options.length === 0 && !loading && !error) {
      ...
      DISKAPI.getQuestions()
        .then((questions: Question[]) => {
          ...
          if (selectedId) {
            let selectedQuestion: Question = questions.filter(
              (q) => q.id === selectedId
            )?.[0];
            if (selectedQuestion) onQuestionChange(selectedQuestion);
            else
  ...
  React.useEffect(() => {
    if (selectedId && options.length > 0) {
      let selectedQuestion: Question = options.filter(
        (q) => q.id === selectedId
      )?.[0];
      if (selectedQuestion) onQuestionChange(selectedQuestion);
      else console.warn("Selected question not found on question catalog");
    }
  ...
  const onQuestionChange = (value: Question | null) => {
    ...
    if (notifyChange)
      if (value)
        notifyChange(
          value,
          value.variables.map((v: QuestionVariable) => v.varName)
        );
    ...
    <Box>
      ...
      <Card
        ...
        sx={{
          mt: "8px",
          p: "0px 10px 10px;",
          display: questionParts.length > 0 ? "block" : "none",
          position: "relative",
          overflow: "visible",
        }}
      >
        ...
        >
          This line of inquiry can be used to investigate the following
        ...
        >
          {questionParts.length > 0
            ? questionParts.map((part: string, i: number, parts: string[]) =>
                part.charAt(0) !== "?" ? (
